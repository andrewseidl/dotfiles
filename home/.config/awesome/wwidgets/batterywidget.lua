...
function batterywidget.setup(settings)
    ...
    local color_powersave = settings.color_powersave or "#00ff00"
    ...
    vicious.register(txt, vicious.widgets.bat, function(widget, args)
        ...
        if capacity < 8 and loading == "−" then
            ...
            return '<span color="' .. color_low .. '">' .. args[3] .. ' ' .. capacity .. '%</span>'
        else
            return '<span color="' .. color_fg .. '">' .. args[3] .. ' ' .. capacity .. '%</span>'
        end
    end, 61, "BAT0")
